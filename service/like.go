...
func QueryLikeCountListByVideoIdList(videoIdList *[]int64) (likeCountList []entity.VideoLikeCnt, err error) {
	result := global.DB.Model(&entity.Like{}).Select("video_id", "count(video_id) as like_cnt").Where("video_id in ?", *videoIdList).Group("video_id").Find(&likeCountList)
	if result.Error != nil {
...
func GetLikeVideoListByUserId(userId int64, currentId int64) (videos []entity.VideoResponse, err error) {
	...
	for i, video := range likeVideoList {
		...
		videos[i].FavoriteCount = likeCountList[i].LikeCnt
		videos[i].CommentCount = commentCountList[i].CommentCnt
	}
